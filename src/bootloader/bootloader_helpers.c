...
int _bootloader_retrieve_ctxt(struct bootloader_ctxt_t *bctxt, struct
...
bool _bootloader_ctxt_equal(struct bootloader_ctxt_t *b1, struct
			    bootloader_ctxt_t *b2)
{
	if (!memcmp(b1, b2, sizeof(struct bootloader_ctxt_t))) {
		return true;
	}
	return false;
}
			   ...
			   storage_driver_t *sdriver)
{
	...
	bctxt->crc = crc_run_crc32((uint8_t *)bctxt, (sizeof(struct
							     bootloader_ctxt_t) - sizeof(bctxt->crc)));
...
int _bootloader_initialize_ctxt(struct bootloader_ctxt_t *bctxt, struct
				storage_driver_t *sdriver)
{
	LOG_INFO("Creating new bootloader context");
...
int _bootloader_send_bootinfo(struct bootloader_ctxt_t *bctxt, struct comm_driver_t *cdriver)
{
	...
	memset(out_buffer, 0, COMMP_CMD_PACKET_SIZE + sizeof(struct bootloader_ctxt_t));
	...
	return comm_protocol_write_data(cdriver, out_buffer, sizeof(struct bootloader_ctxt_t));
}
