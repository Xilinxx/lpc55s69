...
int _comm_send_rrq(struct comm_driver_t * cdriver, struct comm_ctxt_t * ctxt) {
    ...
    if (ctxt->part_nr == 0) {
        ...
        size_of_romname = sizeof(COMMP_ROMNAME_ROM1);
    ...
    } else if (ctxt->part_nr == COMMP_ROMID_SPIFLASH1) {
        memcpy(rrq.romname, COMMP_ROMNAME_SPIFLASH1, sizeof(COMMP_ROMNAME_SPIFLASH1));
        size_of_romname = sizeof(COMMP_ROMNAME_SPIFLASH1);
    } else {
...
int _comm_send_wrq(struct comm_driver_t * cdriver,
    ...
    if (ctxt->part_nr == COMMP_ROMID_ROM0) {
        ...
        memcpy(wrq.romname, COMMP_ROMNAME_SPIFLASH1, sizeof(COMMP_ROMNAME_SPIFLASH1));
    } else {
