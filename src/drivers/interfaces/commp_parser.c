...
static int _rrq_phandler(void * priv, uint8_t * data, size_t size) {
    ...
    } else if (!strcmp((char *)rrq.romname, COMMP_ROMNAME_SPIFLASH1)) {
        ctxt->part_nr = COMMP_ROMID_SPIFLASH1;
    } else {
...
static int _wrq_phandler(void * priv, uint8_t * data, size_t size) {
    ...
    if (!strcmp((char *)wrq.romname, COMMP_ROMNAME_ROM0)) {
        ...
        ctxt->part_nr = COMMP_ROMID_SPIFLASH0;
        erase = false;
    } else if (!strcmp((char *)wrq.romname, COMMP_ROMNAME_SPIFLASH1)) {
        ctxt->part_nr = COMMP_ROMID_SPIFLASH1;
        erase = false;
    } else {
    ...
    if (ctxt->parent && ctxt->parent->sdriver && erase == true) {
...
static int _cmd_phandler(void * priv, uint8_t * data, size_t size) {
    ...
    if (found) {
        LOG_INFO("Command code: %d - [%s]", cmd.cmdcode,
                 comm_protocol_cmd_to_string(cmd.cmdcode));
    } else {
...
const char * comm_protocol_cmd_to_string(comm_proto_cmd_t cmd) {
    ...
    switch (cmd) {
        case COMMP_CMD_BOOT: strcpy(str, "CMD_BOOT");
            break;
        ...
        case COMMP_CMD_BOOTINFO: strcpy(str, "CMD_INFO");
            break;
        case COMMP_CMD_PWR_ON: strcpy(str, "CMD_PWR_ON");
            break;
        case COMMP_CMD_PWR_OFF: strcpy(str, "CMD_PWR_OFF");
            break;
        case COMMP_CMD_RECONFIG_GOWIN: strcpy(str, "CMD_RECONFIG_GOWIN");
            break;
        case COMMP_CMD_END: strcpy(str, "CMD_END");
            break;
        case COMMP_CMD_SPI_END: strcpy(str, "CMD_SPI_END");
            break;
        case COMMP_CMD_RESET: strcpy(str, "CMD_RESET");
            break;
        case COMMP_CMD_TRIGGER_WDOG: strcpy(str, "CMD_WDOG");
            break;
        case COMMP_CMD_ERASE_SPI0: strcpy(str, "CMD_ERASE_SPI0");
            break;
        case COMMP_CMD_ERASE_SPI1: strcpy(str, "CMD_ERASE_SPI1");
            break;
        case COMMP_CMD_WRITE_SPI_CTXT: strcpy(str, "CMD_WRITE_SPI_CTXT");
            break;
        case COMMP_CMD_INFO_SPI: strcpy(str, "CMD_SPI_INFO");
            break;
        case COMMP_CMD_SET_SPI0: strcpy(str, "CMD_SET_SPI0");
            break;
        case COMMP_CMD_SET_SPI1: strcpy(str, "CMD_SET_SPI1");
            break;
        case COMMP_CMD_SET_ROM0: strcpy(str, "CMD_SET_ROM0");
            break;
