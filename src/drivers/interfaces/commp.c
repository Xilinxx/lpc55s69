...
/* Helpers from commp_cmd_helpers.c */
											...
											struct storage_driver_t * spidriver)
{
	...
	while (!end_stack) {
		...
		err = comm_protocol_parse_packet(&transfer_ctxt, recv_buffer, readlen);
		if (err < 0) {
			LOG_ERROR("Failed to parse packet");
			_reset_tranfer_ctxt(&transfer_ctxt);
			return COMMP_CMD_ERROR;
		}
		...
		if (comm_protocol_is_packet_type(recv_buffer,COMMP_RRQ)) {
			...
			transfer_ctxt.rom_readsize = (size_t)(recv_buffer[COMMP_RRQ_LENGTH_OFFSET] << 24) +
																	 (recv_buffer[COMMP_RRQ_LENGTH_OFFSET + 1] << 16) +
																	 (recv_buffer[COMMP_RRQ_LENGTH_OFFSET + 2] << 8) +
																	 (recv_buffer[COMMP_RRQ_LENGTH_OFFSET + 3]);
			char device[readlen];
			memcpy(device, recv_buffer+COMMP_RRQ_ROMNAME_OFFSET, 5);
			transfer_ctxt.part_nr = COMMP_ROMID_SPIFLASH0; // spi0  -> use spidriver
			LOG_INFO("ReadRequest device[%s] size[%d]", device, transfer_ctxt.rom_readsize);
			comm_protocol_retrieve_binary(cdriver, spidriver, &transfer_ctxt);
			transfer_ctxt.transfer_in_progress = false;
		}
		else if (comm_protocol_is_packet_type(recv_buffer,COMMP_WRQ)) {
			LOG_DEBUG("WriteRequest for device [%d]", transfer_ctxt.part_nr);
			needs_ack = true;
		}
		else if (comm_protocol_is_packet_type(recv_buffer, COMMP_DATA)) {
			LOG_DEBUG("Data packet for device [%d]", transfer_ctxt.part_nr);
			needs_ack = true;
		}
			...
			case COMMP_CMD_CRC:
				...
				uint32_t crc = storage_crc_storage(sdriver);
		...
		if (needs_ack) {
			err = _comm_send_ack(cdriver, &transfer_ctxt);
			if (err < 0) {
				return COMMP_CMD_ERROR;
			}
		}
				  ...
				  uint8_t *buffer, size_t len, uint8_t rom_nr, uint32_t crc)
{
	...
	error = _comm_send_wrq(cdriver, &transfer_ctxt); // to rom_nr -> COMMP_ROMID_ROM0
	if (error < 0) {
		_reset_tranfer_ctxt(&transfer_ctxt);
		return -1;
	}
...
{
	int error = _comm_send_boot(cdriver);
	if (error < 0) {
		LOG_ERROR("Failed to send crc");
	}
	return error;
