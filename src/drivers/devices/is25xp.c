...
void is25xp_pagewrite(uint8_t * buffer,
    ...
    if (priv.pageshift != IS25_IS25LP064_PAGE_SHIFT) {
        ...
        LOG_ERROR("private pageshift[%X]<>8 got modified", (long)offset, priv.pageshift);
    }

    // LOG_DEBUG("page: %04lx, offset: %08lx", (long)page, (long)offset);
    ...
    xfer.rxData = destBuff;
    xfer.configFlags = kSPI_FrameAssert;
    xfer.dataSize = 4 + IS25_IS25XP_BYTES_PER_PAGE;
    for (int i = 0; i < IS25_IS25XP_BYTES_PER_PAGE; i++) {
