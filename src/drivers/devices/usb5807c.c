...
static int _usb5807c_i2c_write(uint16_t reg, uint8_t * data, uint16_t count) {
    ...
    status_t error =
        I2C_MasterTransferBlocking(_usb_ctxt.i2c_base, &_usb_ctxt.i2c_transfer);

    switch (error) {
        case kStatus_I2C_Busy:
            LOG_ERROR("Write Previous transmission still not finished.");
            break;
        case kStatus_I2C_Timeout:
            LOG_ERROR("Write Transfer error, wait signal timeout.");
            break;
        case kStatus_I2C_ArbitrationLost:
            LOG_ERROR("Write Transfer error, arbitration lost.");
            break;
        case kStatus_I2C_Nak:
            LOG_ERROR("Write Transfer error, receive NAK during transfer.");
            break;
        case kStatus_I2C_Addr_Nak:
            LOG_ERROR("Write Transfer error, receive NAK during addressing. Reg %x", reg);
            break;
        default:
            break;
    }
...
static int _usb5807c_i2c_read(uint16_t reg, uint8_t * data, uint8_t count) {
    ...
    switch (error) {
        case kStatus_I2C_Busy:
            LOG_ERROR("Read Previous transmission still not finished.");
            break;
        case kStatus_I2C_Timeout:
            LOG_ERROR("Read Transfer error, wait signal timeout.");
            break;
        case kStatus_I2C_ArbitrationLost:
            LOG_ERROR("Read Transfer error, arbitration lost.");
            break;
        case kStatus_I2C_Nak:
            LOG_ERROR("Read Transfer error, receive NAK during transfer.");
            break;
        case kStatus_I2C_Addr_Nak:
            LOG_ERROR("Read Transfer error, receive NAK during addressing. Reg %x", reg);
            break;
        default:
            break;
    }
