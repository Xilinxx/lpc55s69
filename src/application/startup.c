...
#define MAX_REBOOT_RETRIES 250
    ...
    switch (_gdata.application_request) {
        case UPDATE:
            LOG_INFO("application request to jump to bootcode");
            break;
        default:
            if (_gdata.reboot_counter < MAX_REBOOT_RETRIES) {
                LOG_INFO("ResetISR->start application");
                // jumps to same application code
                _gdata.err_code = (uint32_t)main();
            } else {
                LOG_WARN("Max app reboot retries %d reached.", MAX_REBOOT_RETRIES);
            }
    }
