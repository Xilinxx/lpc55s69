...
void commp_stack_should_fail_tests(void ** state) {
    ...
    error = comm_protocol_run(&_bctxt, NULL, NULL, NULL, &_spictxt);
    assert_int_equal(error, -1);
    ...
    error = comm_protocol_run(&_bctxt, drv, NULL, NULL, &_spictxt);
    assert_int_equal(error, -1);
...
void commp_stack_should_pass_tests(void ** state) {
    ...
    for (int i = 1; i < 100; i++) {
        will_return(__wrapper_unit_read, COMMP_DATA);
        will_return(__wrapper_unit_read, true);         // !< full packet
        will_return(__wrapper_unit_read, i);         // !< packet nr
    }

    will_return(__wrapper_unit_read, COMMP_CMD);
    will_return(__wrapper_unit_read, COMMP_CMD_CRC);
    will_return(__wrapper_unit_read, COMMP_ROMNAME_ROM0);     // set parameter to recognize rom

    error = comm_protocol_run(&_bctxt, drv, NULL, NULL, &_spictxt);
    assert_return_code(error, 0);
    assert_int_equal(error, COMMP_CMD_CRC);
    ...
    error = comm_protocol_run(&_bctxt, drv, NULL, NULL, &_spictxt);
    assert_return_code(error, 0);
    assert_int_equal(error, COMMP_CMD_END);
...
void commp_stack_write_spi0_should_pass_tests(void ** state) {
    LOG_RAW("");
    LOG_INFO("Testing COMMP Stack WRITE_WRQ SPI0 (should pass)");
    ...
    will_return(__wrapper_unit_read, COMMP_WRQ);
    will_return(__wrapper_unit_read, "spi0");
    ...
    will_return(__wrapper_unit_read, COMMP_CMD);
    will_return(__wrapper_unit_read, COMMP_CMD_CRC);
    will_return(__wrapper_unit_read, COMMP_ROMID_SPIFLASH0);     // set parameter to recognize spi

    // !< returns COMMP_CMD_ERROR if CRC is wrong
    error = comm_protocol_run(&_bctxt, drv, NULL, NULL, &_spictxt);
    assert_return_code(error, 0);
    // !< Specific return value for triggering context storage of spi
    assert_int_equal(error, COMMP_CMD_WRITE_SPI_CTXT);
    ...
    LOG_OK("Stack tests WRITE_WRQ SPI0 passed (should pass)");
}
...
void commp_stack_long_term_stress_test() {
    ...
    will_return(__wrapper_unit_read, COMMP_CMD);
    will_return(__wrapper_unit_read, COMMP_CMD_CRC);
    will_return(__wrapper_unit_read, COMMP_ROMNAME_ROM0);     // set parameter to recognize approm
...
int main(void) {
    ...
    const struct CMUnitTest tests_that_should_pass[] = {
        ...
        cmocka_unit_test(commp_stack_write_spi0_should_pass_tests),
    };
