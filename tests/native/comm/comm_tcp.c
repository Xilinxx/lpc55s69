...
static int _comm_tcp_init(void * drv) {
...
static int _comm_tcp_write(void * drv, uint8_t * buffer, size_t len) {
    struct comm_driver_t * driver = (struct comm_driver_t *)drv;
...
static int _comm_tcp_read(void * drv, uint8_t * buffer, size_t * len) {
    struct comm_driver_t * driver = (struct comm_driver_t *)drv;
    ...
    if (read != *len) {
        LOG_WARN("Did not read requested length (%d - %d)",
                 read, *len);
    }
    *len = read;
...
static void _comm_tcp_close(void * drv) {
    return;
}

static const struct comm_ops_t _tcp_ops = {
    .init  = _comm_tcp_init,
    .write = _comm_tcp_write,
    .read  = _comm_tcp_read,
    .close = _comm_tcp_close,
};

struct comm_driver_t tcp_comm = {
    .enabled   = true,
    .name      = "tcp",
    .ops       = &_tcp_ops,
    .priv_data = NULL,
