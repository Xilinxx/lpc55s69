...
#if defined(CRC_DRIVER_USE_CRC16_CCITT_FALSE_AS_DEFAULT) && \
...
void CRC_Init(CRC_Type *base, const crc_config_t *config)
{
	...
	base->MODE = CRC_MODE_CRC_POLY(config->polynomial) |
		     CRC_MODE_BIT_RVS_WR(config->reverseIn) |
		     CRC_MODE_CMPL_WR(config->complementIn) |
		     CRC_MODE_BIT_RVS_SUM(config->reverseOut) |
...
void CRC_GetDefaultConfig(crc_config_t *config)
{
	...
	static const crc_config_t default_config =
	{ CRC_DRIVER_DEFAULT_POLYNOMIAL,
	  CRC_DRIVER_DEFAULT_REVERSE_IN,
	  CRC_DRIVER_DEFAULT_COMPLEMENT_IN,
	  CRC_DRIVER_DEFAULT_REVERSE_OUT,
	  CRC_DRIVER_DEFAULT_COMPLEMENT_OUT,
	  CRC_DRIVER_DEFAULT_SEED };
...
void CRC_GetConfig(CRC_Type *base, crc_config_t *config)
{
	...
	config->polynomial =
		(crc_polynomial_t)(uint32_t)(((uint32_t)(mode &
	...
	base->MODE = mode &
		     ~((1U << CRC_MODE_BIT_RVS_SUM_SHIFT) |
		       (1U << CRC_MODE_CMPL_SUM_SHIFT));
...
#pragma GCC diagnostic ignored "-Wcast-align"
	data32 = (const uint32_t *)data; // -> warning: cast increases required alignment of target type
