...
#define SPI_FIFOWR_FLAGS_MASK (~(SPI_DEASSERT_ALL | SPI_FIFOWR_TXDATA_MASK | \
				 SPI_FIFOWR_LEN_MASK))

#define SPI_FIFOTRIG_TXLVL_GET(base) (((base)->FIFOTRIG & \
				       SPI_FIFOTRIG_TXLVL_MASK) >> \
				      SPI_FIFOTRIG_TXLVL_SHIFT)
#define SPI_FIFOTRIG_RXLVL_GET(base) (((base)->FIFOTRIG & \
...
typedef enum _spi_spol {
	...
	kSPI_SpolActiveAllHigh	=
		(kSPI_Spol0ActiveHigh | kSPI_Spol1ActiveHigh |
		 kSPI_Spol2ActiveHigh),
#else
	kSPI_Spol3ActiveHigh	= SPI_CFG_SPOL3(1),
	kSPI_SpolActiveAllHigh	=
		(kSPI_Spol0ActiveHigh | kSPI_Spol1ActiveHigh |
		 kSPI_Spol2ActiveHigh | kSPI_Spol3ActiveHigh),
#endif
...
typedef void (*spi_master_callback_t)(SPI_Type *base,
				      spi_master_handle_t *handle, status_t
...
typedef void (*flexcomm_spi_master_irq_handler_t)(SPI_Type *		base,
						  spi_master_handle_t * handle);

/*! @brief Typedef for slave interrupt handler. */
typedef void (*flexcomm_spi_slave_irq_handler_t)(SPI_Type *		base,
 ...
 */
status_t SPI_MasterSetBaud(SPI_Type *base, uint32_t baudrate_Bps, uint32_t
			   srcClock_Hz);
 ...
 */
static inline void SPI_SetTransferDelay(SPI_Type *			base,
					const spi_delay_config_t *	config)
{
	assert(NULL != base);
	assert(NULL != config);
	base->DLY =
		(SPI_DLY_PRE_DELAY(config->preDelay) |
		 SPI_DLY_POST_DELAY(config->postDelay) |
		 SPI_DLY_FRAME_DELAY(config->frameDelay) |
		 SPI_DLY_TRANSFER_DELAY(config->transferDelay));
}
 ...
 * @param userData User data.
 */
static inline status_t SPI_SlaveTransferCreateHandle(
	SPI_Type *base,
	spi_slave_handle_t *
	handle,
 ...
 * @return status of status_t.
 */
static inline status_t SPI_SlaveTransferGetCount(SPI_Type *		base,
						 spi_slave_handle_t *	handle,
						 size_t *		count)
{
	return SPI_MasterTransferGetCount(base, (spi_master_handle_t *)handle,
					  count);
}
 ...
 */
static inline void SPI_SlaveTransferAbort(SPI_Type *		base,
					  spi_slave_handle_t *	handle)
{
