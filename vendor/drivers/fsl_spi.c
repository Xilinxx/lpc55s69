...
#else
#define SPI_FIFO_DEPTH(base) ((((base)->FIFOCFG & SPI_FIFOCFG_SIZE_MASK) >> \
			       SPI_FIFOCFG_SIZE_SHIFT) << 3)
...
static spi_config_t g_configs[FSL_FEATURE_SOC_SPI_COUNT] =
{ (spi_data_width_t)0 };

/*! @brief Array to map SPI instance number to base address. */
static const uint32_t s_spiBaseAddrs[FSL_FEATURE_SOC_SPI_COUNT] =
	SPI_BASE_ADDRS;
 ...
 */
status_t SPI_MasterInit(SPI_Type *base, const spi_master_config_t *config,
			uint32_t srcClock_Hz)
{
	...
	tmp = base->CFG;
	tmp &=
		~(SPI_CFG_MASTER_MASK | SPI_CFG_LSBF_MASK | SPI_CFG_CPHA_MASK |
		  SPI_CFG_CPOL_MASK | SPI_CFG_LOOP_MASK |
	...
	tmp = base->FIFOTRIG &
	      (~(SPI_FIFOTRIG_RXLVL_MASK | SPI_FIFOTRIG_TXLVL_MASK));
	tmp |= SPI_FIFOTRIG_TXLVL(config->txWatermark) | SPI_FIFOTRIG_RXLVL(
		config->rxWatermark);
...
status_t SPI_SlaveInit(SPI_Type *base, const spi_slave_config_t *config)
{
	...
	tmp &=
		~(SPI_CFG_MASTER_MASK | SPI_CFG_LSBF_MASK | SPI_CFG_CPHA_MASK |
		  SPI_CFG_CPOL_MASK | SPI_CFG_ENABLE_MASK |
		  SPI_SSELPOL_MASK);
	/* phase */
	tmp |= SPI_CFG_CPHA(config->phase);
	/* polarity */
...
void SPI_Deinit(SPI_Type *base)
{
	...
	base->FIFOINTENCLR = SPI_FIFOINTENCLR_TXERR_MASK |
 ...
 */
status_t SPI_MasterSetBaud(SPI_Type *base, uint32_t baudrate_Bps,
			   uint32_t srcClock_Hz)
{
	uint32_t tmp;
	...
	control |=
		(SPI_DEASSERT_ALL &
		 (~SPI_DEASSERTNUM_SSEL(
			  (uint32_t)(g_configs[instance].sselNum))));
	/* mask configFlags */
...
status_t SPI_MasterTransferBlocking(SPI_Type *base, spi_transfer_t *xfer)
{
	...
	/* check params */
	assert(!((NULL == base) || (NULL == xfer) ||
		 ((NULL == xfer->txData) && (NULL == xfer->rxData))));
	...
	/* delay for frames */
	tx_ctrl |=
		((xfer->configFlags & (uint32_t)kSPI_FrameDelay) !=
		 0U) ? (uint32_t)kSPI_FrameDelay : 0U;
	/* end of transfer */
	last_ctrl |=
		((xfer->configFlags & (uint32_t)kSPI_FrameAssert) !=
		 0U) ? (uint32_t)kSPI_FrameAssert : 0U;
	/* last index of loop */
	while ((txRemainingBytes != 0U) || (rxRemainingBytes != 0U) ||
		...
		if (((base->FIFOSTAT & SPI_FIFOSTAT_TXNOTFULL_MASK) != 0U) &&
		    (toReceiveCount < fifoDepth) &&
		    ((txRemainingBytes != 0U) ||
		     (rxRemainingBytes >=
		      SPI_COUNT_TO_BYTES(dataWidth, toReceiveCount + 1U)))) {
				       ...
				       spi_transfer_t *		xfer)
{
	/* check params */
	assert(
		!((NULL == base) || (NULL == handle) || (NULL == xfer) ||
		  ((NULL == xfer->txData) && (NULL == xfer->rxData))));
	...
	/* dataSize (in bytes) is not aligned to 16bit (2B) transfer */
	assert(!((handle->dataWidth > (uint8_t)kSPI_Data8Bits) &&
		 ((xfer->dataSize & 0x1U) != 0U)));
 ...
 */
status_t SPI_MasterHalfDuplexTransferBlocking(
	SPI_Type *base,
 ...
 * return status of status_t.
 */
status_t SPI_MasterTransferGetCount(SPI_Type *base, spi_master_handle_t *handle,
				    size_t *count)
{
	(void)base;
	assert(NULL != handle);
...
void SPI_MasterTransferAbort(SPI_Type *base, spi_master_handle_t *handle)
{
	...
	base->FIFOINTENSET &=
		~(SPI_FIFOINTENSET_TXLVL_MASK | SPI_FIFOINTENSET_RXLVL_MASK);
	/* Empty FIFOs */
	base->FIFOCFG |= SPI_FIFOCFG_EMPTYTX_MASK | SPI_FIFOCFG_EMPTYRX_MASK;
...
static void SPI_TransferHandleIRQInternal(SPI_Type *		base,
					  spi_master_handle_t * handle)
{
	...
	assert((NULL != base) && (NULL != handle) &&
	       ((NULL != handle->txData) || (NULL != handle->rxData)));

	fifoDepth = SPI_FIFO_DEPTH(base);
	/* select slave to talk with */
	tx_ctrl |=
		((uint32_t)SPI_DEASSERT_ALL &
		 (uint32_t)SPI_ASSERTNUM_SSEL(handle->sselNum));
	/* set width of data */
	tx_ctrl |= SPI_FIFOWR_LEN(handle->dataWidth);
	/* delay for frames */
	tx_ctrl |=
		((handle->configFlags & (uint32_t)kSPI_FrameDelay) !=
		 0U) ? (uint32_t)kSPI_FrameDelay : 0U;
	/* end of transfer */
	last_ctrl |=
		((handle->configFlags & (uint32_t)kSPI_FrameAssert) !=
		 0U) ? (uint32_t)kSPI_FrameAssert : 0U;
	do{
		loopContinue = false;
		...
		if (((base->FIFOSTAT & SPI_FIFOSTAT_TXNOTFULL_MASK) != 0U) &&
		    ((uint32_t)toReceiveCount < fifoDepth) &&
		    ((txRemainingBytes != 0U) ||
		     (rxRemainingBytes >=
		      SPI_COUNT_TO_BYTES(handle->dataWidth,
					 toReceiveCount + 1U)))) {
			/* txBuffer is not empty */
			if ((txRemainingBytes != 0U) &&
			    (handle->txData != NULL)) {
...
void SPI_MasterTransferHandleIRQ(SPI_Type *base, spi_master_handle_t *handle)
{
	...
	if ((handle->txRemainingBytes != 0U) ||
		...
		if ((0U == handle->txRemainingBytes) &&
		    (0U == handle->rxRemainingBytes) &&
		    (0U == handle->toReceiveCount)) {
			base->FIFOTRIG = base->FIFOTRIG &
					 (~SPI_FIFOTRIG_TXLVL_MASK);
			base->FIFOINTENSET |= SPI_FIFOINTENSET_TXLVL_MASK;
		} else {
			uint32_t rxRemainingCount = SPI_BYTES_TO_COUNT(
				handle->dataWidth, handle->rxRemainingBytes);
			...
			if ((0U == handle->txRemainingBytes) &&
			    (rxRemainingCount <= handle->toReceiveCount)) {
				base->FIFOINTENCLR =
					SPI_FIFOINTENCLR_TXLVL_MASK;
			}
			...
			if (rxRemainingCount == 0U) {
				...
				if ((txRemainingBytes == 0U) &&
				    (toReceiveCount != 0U) &&
				    (toReceiveCount <
				     SPI_FIFOTRIG_RXLVL_GET(base) + 1U)) {
					base->FIFOTRIG =
						(base->FIFOTRIG &
		...
		base->FIFOTRIG =
			(base->FIFOTRIG &
			 (~(SPI_FIFOTRIG_RXLVL_MASK |
			    SPI_FIFOTRIG_RXLVL_MASK))) |
			SPI_FIFOTRIG_RXLVL(handle->rxWatermark) |
			SPI_FIFOTRIG_TXLVL(handle->txWatermark);
		/* set idle state and call user callback */
		handle->state = (uint32_t)kStatus_SPI_Idle;
		if (handle->callback != NULL) {
			(handle->callback)(base, handle, handle->state,
					   handle->userData);
		}
	}
}
