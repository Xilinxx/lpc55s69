...
static float findPll0MMult(void)
{
	...
	if (SYSCON->PLL0SSCG1 & SYSCON_PLL0SSCG1_SEL_EXT_MASK) {
		mMult =
			(float)((SYSCON->PLL0SSCG1 &
				 SYSCON_PLL0SSCG1_MDIV_EXT_MASK) >>
				SYSCON_PLL0SSCG1_MDIV_EXT_SHIFT);
	} else {
		mMult_int =
			((SYSCON->PLL0SSCG1 & SYSCON_PLL0SSCG1_MD_MBS_MASK) <<
				7U) |
			((SYSCON->PLL0SSCG0) >> PLL_SSCG_MD_INT_P);
		mMult_fract =
			((float)((SYSCON->PLL0SSCG0) & PLL_SSCG_MD_FRACT_M) /
			 (1 << PLL_SSCG_MD_INT_P));
...
static uint32_t CLOCK_GetFro12MFreq(void)
{
	return (ANACTRL->FRO192M_CTRL &
		ANACTRL_FRO192M_CTRL_ENA_12MHZCLK_MASK) ? 12000000U : 0U;
}
...
{
	return (SYSCON->CLOCK_CTRL &
		SYSCON_CLOCK_CTRL_FRO1MHZ_CLK_ENA_MASK) ? 1000000U : 0U;
}
...
static uint32_t CLOCK_GetExtClkFreq(void)
{
	return (ANACTRL->XO32M_CTRL &
		ANACTRL_XO32M_CTRL_ENABLE_SYSTEM_CLK_OUT_MASK) ? CLK_CLK_IN :
	       0U;
}
...
{
	return (ANACTRL->FRO192M_CTRL &
		ANACTRL_FRO192M_CTRL_ENA_96MHZCLK_MASK) ? 96000000U : 0U;
}
...
{
	return ((!(PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_FRO32K_MASK)) &&
...
void SystemCoreClockUpdate(void)
{
	...
	case 0x02:              /* PLL1 clock (pll1_clk)*/
		...
		if (((SYSCON->PLL1CTRL & SYSCON_PLL1CTRL_BYPASSPLL_MASK) ==
			...
			/* MDEC used for rate */
			workRate1 = (uint64_t)clkRate *
				    (uint64_t)findPll1MMult();
