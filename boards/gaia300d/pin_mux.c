...
void BOARD_SetupUsbPins(void)
{
  ...
  GPIO_PinWrite(GPIO, 0, 29, 1); // Reset=0
  GPIO_PinWrite(GPIO, 0, 30, 1); // Reset=0
  GPIO_PinWrite(GPIO, 1, 14, 1); // Reset=0
  GPIO_PinWrite(GPIO, 1, 1, 1);  // Reset=0
  GPIO_PinWrite(GPIO, 0, 28, 0); // Controlled via FET. So GPMCU has to set the its UsbHub0_nVBusUp high to disable Usb Vbus of USB HUB 0. Note the USB HUB 1 is always active.
}

/* BOARD_read_UsbUp0_nVbus
 */
...
{
  GPIO_PinWrite(GPIO, 1, 30, set);
  GPIO_PinWrite(GPIO, 1, 29, set);
}
...
void BOARD_SetupFPGABootPins(void)
{
  ...
  GPIO_PinWrite(GPIO, 1, 22, 0); // Fpga_PsModeTi[2]
  GPIO_PinWrite(GPIO, 1, 23, 0); // Fpga_PsModeTi[1]
  GPIO_PinWrite(GPIO, 1, 24, 1); // Fpga_PsModeTi[0]

  /* Set eMMC18 boot 0110 */
  /* Set USB boot    0111 */

  GPIO_PinInit(GPIO, 1, 20, &outputconf);
  GPIO_PinWrite(GPIO, 1, 20, 0); // OutputEnable, correct???
}
